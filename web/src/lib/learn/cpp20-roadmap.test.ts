...
import { buildCpp20Roadmap } from "./cpp20-roadmap";

const manifest = contentManifestSchema.parse(manifestJson);
const source = parseYaml(
  readFileSync(
    path.join(process.cwd(), "content", "roadmaps", "cpp20.yaml"),
    "utf8",
  ),
);

describe("C++20 roadmap", () => {
  it("contains the complete 52-day curriculum in eight ordered phases", () => {
    const roadmap = buildCpp20Roadmap(source, "vi", manifest.lessons);

    expect(roadmap.phases).toHaveLength(8);
    expect(roadmap.days).toHaveLength(52);
    expect(roadmap.days.map((entry) => entry.day)).toEqual(
      Array.from({ length: 52 }, (_, index) => index + 1),
    );
    expect(roadmap.phases.flatMap((phase) => phase.days)).toHaveLength(52);
  });

  it("only links published C++20 lessons and never creates dead links for planned days", () => {
    const roadmap = buildCpp20Roadmap(source, "vi", manifest.lessons);
    const cpp20LessonIds = new Set(
      manifest.lessons
    ...
    for (const entry of roadmap.days) {
      for (const lesson of entry.lessons) {
        expect(cpp20LessonIds.has(lesson.id)).toBe(true);
      }
  ...
  it("localizes roadmap-owned copy without changing linked lesson identity", () => {
    const vietnamese = buildCpp20Roadmap(source, "vi", manifest.lessons);
    const english = buildCpp20Roadmap(source, "en", manifest.lessons);

    expect(vietnamese.days[0].title).toBe(
      "Toolchain, cờ compiler và chế độ C++20",
    );
    expect(english.days[0].title).toBe(
      "Toolchain, Compiler Flags, and C++20 Mode",
    );
  ...
  it("reports readiness as content coverage rather than learner progress", () => {
    const roadmap = buildCpp20Roadmap(source, "en", manifest.lessons);
