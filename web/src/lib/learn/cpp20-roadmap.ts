...
const cpp20RoadmapSourceSchema = z.object({
  schemaVersion: z.literal(1),
  track: z.literal("cpp20"),
  phases: z.array(
    z.object({
...
type Cpp20RoadmapSource = z.infer<typeof cpp20RoadmapSourceSchema>;
export type RoadmapCoverage = z.infer<typeof coverageSchema>;
export type RoadmapLocale = z.infer<typeof roadmapLocaleSchema>;

export type Cpp20RoadmapLesson = {
  id: string;
  title: string;
};

export type Cpp20RoadmapDay = {
  day: number;
  phaseId: string;
  title: string;
  objective: string;
  dependsOn: number[];
  lessons: Cpp20RoadmapLesson[];
  coverage: RoadmapCoverage;
};

export type Cpp20RoadmapPhase = {
  id: string;
  order: number;
  title: string;
  summary: string;
  days: Cpp20RoadmapDay[];
};

export type Cpp20Roadmap = {
  track: "cpp20";
  phases: Cpp20RoadmapPhase[];
  days: Cpp20RoadmapDay[];
...
function validateRoadmapSource(
  ...
  const expectedDays = Array.from({ length: 52 }, (_, index) => index + 1);
  const actualDays = source.days.map((entry) => entry.day).sort((a, b) => a - b);
  if (actualDays.join(",") !== expectedDays.join(",")) {
    throw new Error("The C++20 roadmap must contain every day from 1 through 52 exactly once");
  }
  ...
  for (const entry of source.days) {
    ...
    for (const lessonId of entry.lessonIds) {
      ...
      if (lesson.track !== "cpp20") {
        throw new Error(`Roadmap day ${entry.day} links non-C++20 lesson ${lessonId}`);
      }
    }
  }
...
export function buildCpp20Roadmap(
  input: unknown,
  localeInput: string,
  lessons: readonly Pick<GeneratedLesson, "id" | "title" | "track">[],
): Cpp20Roadmap {
  const source = cpp20RoadmapSourceSchema.parse(input);
  ...
  const daysByPhase = new Map<string, Cpp20RoadmapDay[]>();
...
function roadmapFilePath() {
  ...
  return path.basename(workingDirectory).toLowerCase() === "web"
    ? path.join(workingDirectory, "content", "roadmaps", "cpp20.yaml")
    : path.join(workingDirectory, "web", "content", "roadmaps", "cpp20.yaml");
}

export async function loadCpp20Roadmap(
  locale: string,
  lessons: readonly Pick<GeneratedLesson, "id" | "title" | "track">[],
) {
  const source = parseYaml(await readFile(roadmapFilePath(), "utf8"));
  return buildCpp20Roadmap(source, locale, lessons);
}
