...
const cpp11RoadmapSourceSchema = z.object({
  schemaVersion: z.literal(1),
  track: z.literal("cpp11"),
  phases: z.array(
    z.object({
...
type Cpp11RoadmapSource = z.infer<typeof cpp11RoadmapSourceSchema>;
export type RoadmapCoverage = z.infer<typeof coverageSchema>;
export type RoadmapLocale = z.infer<typeof roadmapLocaleSchema>;

export type Cpp11RoadmapLesson = {
  id: string;
  title: string;
};

export type Cpp11RoadmapDay = {
  day: number;
  phaseId: string;
  title: string;
  objective: string;
  dependsOn: number[];
  lessons: Cpp11RoadmapLesson[];
  coverage: RoadmapCoverage;
};

export type Cpp11RoadmapPhase = {
  id: string;
  order: number;
  title: string;
  summary: string;
  days: Cpp11RoadmapDay[];
};

export type Cpp11Roadmap = {
  track: "cpp11";
  phases: Cpp11RoadmapPhase[];
  days: Cpp11RoadmapDay[];
...
function validateRoadmapSource(
  ...
  const expectedDays = Array.from({ length: 53 }, (_, index) => index + 1);
  const actualDays = source.days.map((entry) => entry.day).sort((a, b) => a - b);
  if (actualDays.join(",") !== expectedDays.join(",")) {
    throw new Error("The C++11 roadmap must contain every day from 1 through 53 exactly once");
  }
  ...
  for (const entry of source.days) {
    ...
    for (const lessonId of entry.lessonIds) {
      ...
      if (lesson.track !== "cpp11") {
        throw new Error(`Roadmap day ${entry.day} links non-C++11 lesson ${lessonId}`);
      }
    }
  }
...
export function buildCpp11Roadmap(
  input: unknown,
  localeInput: string,
  lessons: readonly Pick<GeneratedLesson, "id" | "title" | "track">[],
): Cpp11Roadmap {
  const source = cpp11RoadmapSourceSchema.parse(input);
  ...
  const daysByPhase = new Map<string, Cpp11RoadmapDay[]>();
...
function roadmapFilePath() {
  ...
  return path.basename(workingDirectory).toLowerCase() === "web"
    ? path.join(workingDirectory, "content", "roadmaps", "cpp11.yaml")
    : path.join(workingDirectory, "web", "content", "roadmaps", "cpp11.yaml");
}

export async function loadCpp11Roadmap(
  locale: string,
  lessons: readonly Pick<GeneratedLesson, "id" | "title" | "track">[],
) {
  const source = parseYaml(await readFile(roadmapFilePath(), "utf8"));
  return buildCpp11Roadmap(source, locale, lessons);
}
