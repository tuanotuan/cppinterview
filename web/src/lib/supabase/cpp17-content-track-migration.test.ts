...
const migration = readFileSync(
  path.join(
    ...
    "supabase",
    "migrations",
    "20260829130000_add_cpp17_content_track.sql",
  ),
  "utf8",
).toLowerCase();

describe("C++17 content-track migration", () => {
  it("extends every current lesson standard and track constraint", () => {
    for (const replacement of [
      "content_lesson_revisions_standard_cpp17_check",
      "content_lesson_revisions_track_cpp17_check",
      "content_lessons_current_standard_cpp17_check",
      "content_lessons_current_track_cpp17_check",
    ]) {
