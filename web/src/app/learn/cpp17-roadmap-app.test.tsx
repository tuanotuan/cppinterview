  ...
  getTranslations: async (namespace: string) =>
    (key: string, values?: Record<string, number | string>) => {
      const messages: Record<string, string> = {
        "Cpp17Roadmap.coverage.ready": "Lesson ready",
        "Cpp17Roadmap.coverage.partial": "Partially covered",
        "Cpp17Roadmap.coverage.planned": "Coming soon",
      };

      if (namespace === "Cpp17Roadmap" && key === "dayLabel") {
        return `Day ${values?.day}`;
      }
      if (namespace === "Cpp17Roadmap" && key === "phaseLabel") {
...
import { Cpp17RoadmapApp } from "./cpp17-roadmap-app";

describe("Cpp17RoadmapApp", () => {
  it("does not repeat the ready status on lesson nodes or in the legend", async () => {
    const html = renderToStaticMarkup(
      await Cpp17RoadmapApp({ roadmap: makeRoadmap("ready") }),
    );
  ...
  it("keeps exceptional coverage visible in the legend", async () => {
    const html = renderToStaticMarkup(
      await Cpp17RoadmapApp({ roadmap: makeRoadmap("partial") }),
    );
...
function makeRoadmap(coverage: RoadmapCoverage): Cpp17Roadmap {
  const day = {
    day: 1,
    phaseId: "foundations",
    title: "Toolchain",
    ...
    dependsOn: [],
    lessons: [{ id: "cpp17-toolchain-compiler-flags-cpp17", title: "Toolchain" }],
    coverage,
  };
