...
import type { Cpp20Roadmap, RoadmapCoverage } from "@/lib/learn/cpp20-roadmap";
  ...
  getTranslations: async (namespace: string) =>
    (key: string, values?: Record<string, number | string>) => {
      const messages: Record<string, string> = {
        "Cpp20Roadmap.coverage.ready": "Lesson ready",
        "Cpp20Roadmap.coverage.partial": "Partially covered",
        "Cpp20Roadmap.coverage.planned": "Coming soon",
      };

      if (namespace === "Cpp20Roadmap" && key === "dayLabel") {
        return `Day ${values?.day}`;
      }
      if (namespace === "Cpp20Roadmap" && key === "phaseLabel") {
...
import { Cpp20RoadmapApp } from "./cpp20-roadmap-app";

describe("Cpp20RoadmapApp", () => {
  it("does not repeat the ready status on lesson nodes or in the legend", async () => {
    const html = renderToStaticMarkup(
      await Cpp20RoadmapApp({ roadmap: makeRoadmap("ready") }),
    );
  ...
  it("keeps exceptional coverage visible in the legend", async () => {
    const html = renderToStaticMarkup(
      await Cpp20RoadmapApp({ roadmap: makeRoadmap("partial") }),
    );
...
function makeRoadmap(coverage: RoadmapCoverage): Cpp20Roadmap {
  const day = {
    day: 1,
    phaseId: "foundations",
    title: "Toolchain",
    ...
    dependsOn: [],
    lessons: [{ id: "cpp20-toolchain-compiler-flags-cpp20", title: "Toolchain" }],
    coverage,
  };
