...
Standard `main` returns `int`; the usual signatures are `int main()` and `int main(int argc, char* argv[])`, with implementation-defined additional forms possible. Falling off the end returns zero, and `argc` includes the program name when it is positive.
...
1. Easy — What is the return value of main and what are the available signatures?
