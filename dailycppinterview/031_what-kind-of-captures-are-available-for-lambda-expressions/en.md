# Question 031: What kind of captures are available for lambda expressions?
...
Captures can be explicit by value or reference, defaulted with `[=]` or `[&]`, include `this` or `*this`, and use init-capture to create a new member of the closure. Choose capture per lifetime and mutation needs rather than defaulting blindly.
...
~~~cpp
auto result = [capture] { return expression; }();
~~~
...
Use this knowledge while reviewing APIs, reading code, explaining diagnostics, or designing abstractions related to lambda expressions. In production, prefer forms that make the contract visible and compiler-checkable.
...
1. Easy — What kind of captures are available for lambda expressions?
