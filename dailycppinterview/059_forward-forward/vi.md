# Câu 059: `std::forward` chuyển tiếp điều gì?
...
`std::forward<T>` cast có điều kiện một đối số forwarding reference theo `T` đã suy luận, giữ việc caller ban đầu truyền lvalue hay rvalue. Chỉ dùng nó cùng template parameter được suy luận tương ứng.
...
1. Dễ — `std::forward` chuyển tiếp điều gì?
