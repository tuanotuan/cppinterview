...
A `const_iterator` allows traversal without mutation through that iterator, works naturally with const containers, and communicates read-only intent to the compiler and reviewer. It does not make the underlying container globally immutable through other aliases.
...
1. Easy — What are the advantages of const_iterators over iterators?
