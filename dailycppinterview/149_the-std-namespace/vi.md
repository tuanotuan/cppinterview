...
`std` là namespace nơi thư viện chuẩn C++ khai báo phần lớn tên như `std::string`, `std::vector` và `std::sort`. Nên định danh bằng `std::` hoặc dùng `using` declaration có phạm vi hẹp; `using namespace std;` ở phạm vi global dễ gây xung đột, và code người dùng không được tự thêm declaration vào `std` ngoài các trường hợp chuẩn cho phép rõ ràng.
