# Question 060: What is the difference between universal and rvalue references?
...
`T&&` is a forwarding reference only when `T` is cv-unqualified and deduced in that context (or for a corresponding `auto&&`). Otherwise it is an ordinary rvalue reference that binds to rvalues; the distinction controls reference collapsing and forwarding.
...
1. Hard — What is the difference between universal and rvalue references?
