...
Return a prvalue directly when possible to obtain guaranteed C++17 elision, or return the same named local from all relevant paths to make NRVO possible. Do not write `std::move(local)` in the return, because it usually prevents NRVO.
...
1. Medium — How can we ensure the compiler performs RVO?
