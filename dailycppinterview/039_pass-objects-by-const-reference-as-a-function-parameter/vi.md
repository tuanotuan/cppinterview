# Câu 039: Có nên truyền object bằng `const` reference không?
...
Dùng `const T&` cho tham số bắt buộc, non-owning, chỉ đọc khi copy `T` tốn kém. Dùng `T` khi hàm cần sở hữu giá trị riêng, `T&` để mutation và dạng pointer khi cần biểu diễn nullability hoặc ownership.
...
1. Trung bình — Có nên truyền object bằng `const` reference không?
