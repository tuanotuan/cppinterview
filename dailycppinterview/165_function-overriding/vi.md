# Câu 165: Function overriding trong C++ là gì và base class ảnh hưởng ra sao?
...
Member của class dẫn xuất override một virtual member của base khi tên, danh sách parameter, cv/ref qualifier cùng quy tắc return và exception tạo thành signature override hợp lệ. Lời gọi qua base reference hoặc pointer khi đó dispatch tới final overrider ở runtime. Nên ghi `override` để compiler bắt mismatch; overload không liên quan nhưng cùng tên có thể che overload set của base, khi đó có thể cần `using Base::name`.
...
1. Trung bình — Function overriding trong C++ là gì và base class ảnh hưởng ra sao?
