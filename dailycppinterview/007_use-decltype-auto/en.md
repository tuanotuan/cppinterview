# Question 007: When to use decltype(auto)?
...
Use `decltype(auto)` when a deduced declaration or return type must preserve exactly what `decltype(expression)` yields, including references. It is powerful for forwarding accessors but can accidentally return a dangling reference.
...
1. Medium — When to use decltype(auto)?
