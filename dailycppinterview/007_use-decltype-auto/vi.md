# Câu 007: Khi nào nên dùng `decltype(auto)`?
...
Dùng `decltype(auto)` khi khai báo hoặc kiểu trả về suy luận phải giữ chính xác kết quả của `decltype(expression)`, kể cả reference. Nó hữu ích cho forwarding accessor nhưng cũng có thể vô tình trả dangling reference.
...
1. Trung bình — Khi nào nên dùng `decltype(auto)`?
