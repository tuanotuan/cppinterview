# Câu 054: Khi nào nên dùng `weak_ptr`?
...
Dùng `std::weak_ptr` làm observer non-owning của object do `shared_ptr` quản lý, đặc biệt để phá ownership cycle. Hãy gọi `lock()` và kiểm tra `shared_ptr` trả về trước khi truy cập object.
...
1. Dễ — Khi nào nên dùng `weak_ptr`?
