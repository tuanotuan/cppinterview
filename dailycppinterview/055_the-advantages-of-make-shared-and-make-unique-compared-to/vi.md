# Câu 055: `std::make_shared` và `std::make_unique` có lợi thế gì so với toán tử `new`?
...
`make_unique` và `make_shared` biểu diễn ownership mà không lộ `new`, đồng thời xây dựng exception-safe. `make_shared` thường gộp object và control block trong một allocation; tuy vậy weak reference có thể giữ allocation gộp lâu hơn và nhu cầu custom deleter có thể khiến cách tạo trực tiếp phù hợp hơn.
...
Dùng kiến thức này khi review API, đọc code, giải thích diagnostic hoặc thiết kế abstraction có liên quan đến smart pointer và ownership. Trong code production, hãy ưu tiên dạng làm contract hiện rõ và được compiler kiểm tra.
...
1. Dễ — `std::make_shared` và `std::make_unique` có lợi thế gì so với toán tử `new`?
