# Câu 121: Từ khóa `mutable` có tác dụng gì?
...
Data member `mutable` có thể được sửa ngay cả qua const object, phù hợp cho cache, counter hoặc mutex logically const mà không đổi giá trị quan sát bên ngoài. Trong lambda, `mutable` cho phép call operator non-const sửa state capture theo giá trị.
...
1. Dễ — Từ khóa `mutable` có tác dụng gì?
