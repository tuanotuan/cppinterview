# Câu 028: Đoạn mã sau in gì và vì sao?
...
Kết quả là `1`. Overload resolution dựa trên cv-qualification của object `a`; vì `a` không const nên hàm member non-const được gọi. `const` trên biến kết quả `b` không ảnh hưởng lời gọi đã xảy ra.
...
1. Trung bình — Đoạn mã sau in gì và vì sao?
