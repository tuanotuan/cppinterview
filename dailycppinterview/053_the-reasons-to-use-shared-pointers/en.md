# Question 053: What are the reasons to use shared pointers?
...
Use `std::shared_ptr` only when several independent owners genuinely determine one lifetime. Reference counting makes ownership explicit, but adds a control block and atomic-count overhead and does not make the pointee itself thread-safe.
...
1. Easy — What are the reasons to use shared pointers?
