# Câu 106: Iterator invalidation là gì? Hãy đưa ra vài ví dụ.
...
Iterator invalidation nghĩa là operation trên container làm iterator, reference hoặc pointer hiện có không còn dùng được. Reallocation của vector vô hiệu toàn bộ handle, erase vô hiệu handle tại hoặc sau vị trí xóa; container dạng node có rule khác cần kiểm tra theo từng operation.
...
Dùng kiến thức này khi review API, đọc code, giải thích diagnostic hoặc thiết kế abstraction có liên quan đến hành vi chương trình và undefined behavior. Trong code production, hãy ưu tiên dạng làm contract hiện rõ và được compiler kiểm tra.
...
1. Khó — Iterator invalidation là gì? Hãy đưa ra vài ví dụ.
