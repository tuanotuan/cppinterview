...
Since C++20, `std::string` and `std::string_view` provide `starts_with` and `ends_with`. Before that, compare a prefix at position zero and compare the suffix only after checking that the source is at least as long as the suffix.
...
1. Easy — How to check if a string starts or ends with a certain substring?
