# Câu 135: Làm sao kiểm tra một string bắt đầu hoặc kết thúc bằng một substring?
...
Từ C++20, `std::string` và `std::string_view` có `starts_with` và `ends_with`. Trước đó, so sánh prefix tại vị trí 0 và chỉ so sánh suffix sau khi kiểm tra chuỗi nguồn dài ít nhất bằng suffix.
...
1. Dễ — Làm sao kiểm tra một string bắt đầu hoặc kết thúc bằng một substring?
