# Câu 098: Khi nào có lý do chính đáng để dùng hàm `init()` khởi tạo object?
...
Hàm `init()` chủ yếu hợp lý với protocol hai pha thật sự, khi construction không thể hoàn tất ngay, chẳng hạn đăng ký cần owner đã được share. Trường hợp khác, nó tạo invalid state có thể quan sát; nên dùng constructor hoặc factory trả object hoàn chỉnh hay lỗi.
...
Dùng kiến thức này khi review API, đọc code, giải thích diagnostic hoặc thiết kế abstraction có liên quan đến thiết kế hướng đối tượng. Trong code production, hãy ưu tiên dạng làm contract hiện rõ và được compiler kiểm tra.
...
1. Trung bình — Khi nào có lý do chính đáng để dùng hàm `init()` khởi tạo object?
