# Câu 020: Nếu kế thừa riêng tư, lớp dẫn xuất có truy cập được thành viên và hàm `public`, `protected` của lớp cơ sở không?
...
Có, implementation của lớp dẫn xuất vẫn truy cập được member `public` và `protected` được kế thừa. Kế thừa riêng tư đổi mức truy cập của chúng qua interface dẫn xuất và chặn implicit public conversion sang base đối với caller bên ngoài.
...
Dùng kiến thức này khi review API, đọc code, giải thích diagnostic hoặc thiết kế abstraction có liên quan đến đa hình, kế thừa và hàm ảo. Trong code production, hãy ưu tiên dạng làm contract hiện rõ và được compiler kiểm tra.
...
1. Trung bình — Nếu kế thừa riêng tư, lớp dẫn xuất có truy cập được thành viên và hàm `public`, `protected` của lớp cơ sở không?
