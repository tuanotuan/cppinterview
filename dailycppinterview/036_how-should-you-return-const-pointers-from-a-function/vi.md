# Câu 036: Nên trả con trỏ `const` từ hàm như thế nào?
...
Trả pointer-to-const, `const T*`, khi caller được quan sát nhưng không được sửa pointee qua handle đó. Top-level const đặt lên chính giá trị pointer trong kiểu trả về sẽ bị bỏ và không ràng buộc caller.
...
1. Trung bình — Nên trả con trỏ `const` từ hàm như thế nào?
