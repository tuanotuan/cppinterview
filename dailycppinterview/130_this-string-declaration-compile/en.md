...
With `<string>` included, `std::string(foo);` is parsed as the declaration `std::string foo;`, so it creates an empty string rather than a discarded temporary. Do not rely on `<iostream>` transitively providing `<string>`; write the intended declaration clearly.
...
1. Hard — Does this string declaration compile?
