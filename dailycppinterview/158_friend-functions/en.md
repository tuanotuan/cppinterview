# Question 158: What is a friend function?
...
A friend function is not a member function, but a `friend` declaration grants it access to a class's private and protected members. Friendship is explicit, neither inherited nor transitive, and does not grant friendship in the reverse direction. It is useful for tightly coupled non-member operations such as symmetric operators, but broad friendship increases coupling and should not replace a well-designed public interface.
...
~~~cpp
class Box { friend double area(const Box& box); private: double width{}; };
~~~
...
1. Medium — What is a friend function?
