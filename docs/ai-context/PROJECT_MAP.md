...
| `/learn` | `[locale]/learn/page.tsx`, `[locale]/learn/[lessonId]/page.tsx`, `app/learn/lesson-ai-assistant.tsx` | Thư viện lesson từ manifest với UI theo locale, tìm kiếm và chip cho các phiên bản C++98/11/14/17/20/23 cùng collection Real-World C++ Interviews. Collection này có reader song ngữ như các lesson khác nhưng khi được chọn, điều hướng không gợi ý hoặc tạo roadmap. Reader render Markdown/code mẫu, đổi VI/EN tại header, đặt CTA kiểm tra ở đầu/cuối bài và có đúng một panel “Học với AI”: mobile nằm trước mục lục/nội dung, desktop sticky bên phải, transcript chỉ ở React memory. Panel kiểm tra `/api/auth/status` sau static render; guest/anonymous chỉ thấy cổng đăng nhập và CTA học thử ba lượt bằng thẻ, không thấy form hỏi AI |
...
track đi Dễ → Trung bình → Khó rồi theo thứ tự manifest; Real-World C++ Interviews đi
