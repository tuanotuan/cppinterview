...
| `/learn` | `[locale]/learn/page.tsx`, `[locale]/learn/[lessonId]/page.tsx`, `app/learn/lesson-ai-assistant.tsx` | Thư viện lesson từ manifest với UI theo locale, tìm kiếm và chip riêng cho từng chuẩn C++98/11/14/17/20/23; chuẩn chưa có lesson bị khóa; card chỉ giữ chuẩn C++, số bài và tiêu đề. Reader render Markdown/code mẫu, đổi VI/EN tại header, đặt CTA kiểm tra ở đầu/cuối bài và có đúng một panel “Học với AI”: mobile nằm trước mục lục/nội dung, desktop sticky bên phải, transcript chỉ ở React memory. Panel kiểm tra `/api/auth/status` sau static render; guest/anonymous chỉ thấy cổng đăng nhập và CTA học thử ba lượt bằng thẻ, không thấy form hỏi AI |
...
chuẩn đi Dễ → Trung bình → Khó rồi theo thứ tự manifest, còn C++98 không tự vào
