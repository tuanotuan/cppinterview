...
| `dailycppinterview/` | Bộ Daily C++ Interview gồm 146 chủ đề theo thứ tự nguồn; mỗi chủ đề có đúng `vi.md`, `en.md`, `main.cpp` và đúng một câu hỏi tự kiểm tra. Đây là collection C++, không phải phiên bản chuẩn và không có roadmap |
...
`cpp17/`, `cpp20/`, `cpp23/`, and the version-neutral `dailycppinterview/`
collection. Migration `20260818110000_archive_non_cpp_content.sql` archives
...
| `/learn` | `[locale]/learn/page.tsx`, `[locale]/learn/[lessonId]/page.tsx`, `app/learn/lesson-ai-assistant.tsx` | Thư viện lesson từ manifest với UI theo locale, tìm kiếm và chip cho các phiên bản C++98/11/14/17/20/23 cùng collection Daily C++ Interview. Daily có reader song ngữ như các lesson khác nhưng khi được chọn, điều hướng không gợi ý hoặc tạo roadmap. Reader render Markdown/code mẫu, đổi VI/EN tại header, đặt CTA kiểm tra ở đầu/cuối bài và có đúng một panel “Học với AI”: mobile nằm trước mục lục/nội dung, desktop sticky bên phải, transcript chỉ ở React memory. Panel kiểm tra `/api/auth/status` sau static render; guest/anonymous chỉ thấy cổng đăng nhập và CTA học thử ba lượt bằng thẻ, không thấy form hỏi AI |
| `/learn/roadmap/cpp11`, `/learn/roadmap/cpp14`, `/learn/roadmap/cpp17`, `/learn/roadmap/cpp20`, `/learn/roadmap/cpp23` | Route locale theo chuẩn, `app/learn/{cpp11,cpp14,cpp17,cpp20,cpp23}-roadmap-app.tsx`, `app/learn/cpp-roadmap-map.tsx`, `lib/learn/{cpp11,cpp14,cpp17,cpp20,cpp23}-roadmap.ts`, `lib/learn/roadmap-progress.ts` | Roadmap song ngữ: C++11 có 53 ngày/8 chặng; C++14 và C++17 đều có 50 ngày/7 chặng; C++20 có 52 ngày/8 chặng; C++23 có 54 ngày/8 chặng. Cả năm chỉ link lesson `ready` đúng track và dùng chung sơ đồ node/connector ba cột ziczac trên desktop, một trục dọc trên tablet/mobile. Node mở lesson chính trong tab mới; toolbar sibling cho phép account đặt `learning`/`done`/`skipped`, còn coverage nội dung vẫn là khái niệm độc lập. Desktop hiện toolbar bằng hover/focus, mobile dùng nút trạng thái 44 px; khách được mời đăng nhập qua Google, GitHub hoặc email. Trang vẫn render tĩnh rồi client hydrate tiến độ account qua API riêng. |
...
| `/stats` | `[locale]/stats/page.tsx`, `[locale]/stats/_components/coverage-dashboard.tsx`, `lib/practice/coverage-analytics.ts` | Dashboard độ phủ theo phiên bản vẫn giới hạn ở ngân hàng canonical C++11/14/17/20/23, độ khó, ma trận chuẩn × độ khó và khoảng trống chủ đề; collection Daily không bị tính thành một phiên bản chuẩn. Lượt học Daily đã xuất bản vẫn đi qua review/progress chung như các thẻ khác |
| `/profile` | `profile/page.tsx`, `lib/profile/{contribution-activity,mobile-usage,profile-activity.server}.ts` | Trang cá nhân và contribution graph 53 tuần từ lượt ôn, AI coach và phỏng vấn thử đã hoàn tất; thẻ Bảo mật đọc capability owner-private để hiện đúng Đặt/Đổi mật khẩu; riêng admin `tuanotuan` còn có tổng thời gian cppinterview hoạt động trên điện thoại hôm nay/7/30 ngày |
...
| `/auth` và `/auth/*` | `auth/page.tsx`, `auth-form.tsx`, `auth-actions.ts`, `auth/{login,callback,confirm,logout,reset-password,set-password}`, `lib/supabase/password-capability.server.ts` | Đăng ký/đăng nhập email-mật khẩu với xác nhận email, khôi phục OTP cho email identity, cùng Google và GitHub OAuth; account không ẩn danh đã đăng nhập có thể đặt hoặc đổi mật khẩu tại `/auth/set-password`, nhận thông báo thành công tại chỗ và giữ phiên hiện tại. `same_password` là kết quả idempotent đã đạt; mật khẩu yếu, phiên cần xác thực lại và rate limit có hướng khôi phục riêng. Capability chính xác đến từ mirror owner-private của `auth.users`, provider metadata chỉ là fallback rolling deploy; mọi Supabase account đã xác thực dùng cppinterview, chỉ admin giữ GitHub identity `tuanotuan` |
...
- `api/roadmap/progress`: xác minh account không ẩn danh bằng JWT claims, chỉ nhận
  ...
  `user_id` từ client; bảng đích còn có owner-only RLS làm lớp bảo vệ thứ hai.
...
- `api/coach/lesson`: bắt buộc account Supabase không ẩn danh, server dựng lại toàn bộ lesson đúng locale từ manifest, gọi Luna với structured answer/citation và không nhận context lesson từ client; guest bị trả `401` trước admission/provider, account non-admin dùng quota AI 3 lượt/24 giờ và owner dùng durable reservation + budget hiện hữu, không fallback Gemini.
...
| `ai` | `public-ai-quota.server.ts`, `public-ai-admission.server.ts`, `public-ai-quota-display.ts`, `public-ai-budget.server.ts` | AI Coach/Mock v5 public và lesson tutor của account non-admin: HMAC IP/device/account, rolling 24 giờ dùng chung, lease chống gọi trùng và ledger Luna site-wide riêng; guest không được vào lesson tutor, không dùng identity thô hay Gemini fallback |
...
registry. Coverage roadmap là trạng thái học liệu. Sau hydration, client gọi
`/api/roadmap/progress` → xác minh claims + membership track →
`user_roadmap_lesson_states` owner-only RLS. `learning` không hoàn tất;
`done + skipped` tính hoàn tất và được thống kê riêng. Chọn lại trạng thái hiện tại
xóa row về pending ngầm. Luồng này không đọc localStorage hay chạm scheduler/FSRS.
...
Daily plan dựng lại deterministic từ exact question identity, trạng thái đầu
ngày và review history của generation hiện hành. Plan lấy mọi
Learning/Relearning đến hạn, tối đa năm Review đến hạn và tối đa năm New theo
thứ tự đó. New đi lần lượt C++11 → C++14 → C++17 → C++20 → C++23; trong mỗi
track đi Dễ → Trung bình → Khó rồi theo thứ tự manifest; Daily C++ Interview đi
sau C++23, còn C++98 không tự vào
daily New. Review cùng ngày chỉ hoàn tất ID đã chọn nên reload/cloud merge không
tự bổ sung quota hoặc làm tăng mẫu số; sang ngày mới, slot trống của năm New
được lấp sau các thẻ hôm trước còn chưa học. Panel Today gộp Relearning vào
“Đang học” và chỉ đếm phần còn lại của plan; bốn learning state vẫn giữ nguyên
cho badge, lọc chi tiết và transition. Repair, Custom Study và Focus là queue
riêng.
...
Lesson reader chỉ hiện form sau khi `/api/auth/status` xác nhận account; guest
hoặc anonymous nhận cổng đăng nhập cùng CTA vào guest Practice. Khi gửi, client
chỉ gửi `lessonId`, locale, transcript giới hạn và UUIDv8 idempotency. Route
`/api/coach/lesson` xác minh lại account ở server trước admission/provider, rồi
localize manifest và serialize đủ mọi
...
Guest và tài khoản không phải owner đi qua public admission riêng ở các surface
công khai; lesson tutor là ngoại lệ chỉ nhận account không ẩn danh. `/practice`
