...
| `/auth` và `/auth/*` | `auth/page.tsx`, `auth-form.tsx`, `auth-actions.ts`, `auth/{login,callback,confirm,logout,reset-password,set-password}` | Đăng ký/đăng nhập email-mật khẩu với xác nhận email, khôi phục OTP cho email identity, cùng Google và GitHub OAuth; account không ẩn danh đã đăng nhập có thể đặt hoặc đổi mật khẩu tại `/auth/set-password`, trong đó `same_password` là kết quả idempotent đã đạt còn mật khẩu yếu, phiên cần xác thực lại và rate limit có hướng khôi phục riêng; mọi Supabase account đã xác thực dùng cppinterview, chỉ admin giữ GitHub identity `tuanotuan` |
